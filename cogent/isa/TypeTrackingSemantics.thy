...
inductive u_tt_sem_pres :: "('f \<Rightarrow> poly_type)
                          ...
                          \<Rightarrow> ('f,'a,'l,'t) uval env
                          \<Rightarrow> lay_env
                          \<Rightarrow> kind env
                          \<Rightarrow> lay_constraints
                          \<Rightarrow> tree_ctx
                          \<Rightarrow> type
                          \<Rightarrow> ('f,'a,'l,'t) store \<times> 'f expr
                          \<Rightarrow> ('f,'a,'l,'t) store \<times> ('f,'a,'l,'t) uval
                          \<Rightarrow> bool"  ("_, _, _, _, _, _, _, _ T\<turnstile> _ \<Down>! _" [30,0,0,0,0,0,0,20] 60)
where
  u_tt_sem_pres_default : "\<lbrakk> \<not> composite_anormal_expr x
                           ; \<xi> , \<gamma> \<turnstile> (\<sigma>, x) \<Down>! st
                           ; \<Xi>, L, K, C, snd \<Gamma> \<turnstile> x : \<tau>
                           ; \<exists>rs ws. matches_ptrs \<Xi> \<sigma> \<gamma> (snd \<Gamma>) rs ws
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, x) \<Down>! st"

| u_tt_sem_pres_let     : "\<lbrakk> ttsplit L K C \<Gamma> sps [] \<Gamma>1 [Some t] \<Gamma>2
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, t T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', a')
                           ; \<Xi>, \<xi> , (a' # \<gamma>), L, K, C, \<Gamma>2, \<tau> T\<turnstile> (\<sigma>', b) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Let a b) \<Down>! st"

| u_tt_sem_pres_letbang : "\<lbrakk> ttsplit_bang is sps L K C \<Gamma> [] \<Gamma>1 [Some t] \<Gamma>2
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, t T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', a')
                           ; \<Xi>, \<xi> , (a' # \<gamma>), L, K, C, \<Gamma>2, \<tau> T\<turnstile> (\<sigma>', b) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, LetBang vs a b) \<Down>! st"

| u_tt_sem_pres_case_m  : "\<lbrakk> ttsplit L K C \<Gamma> sps [] \<Gamma>1 [] \<Gamma>2
                           ; ttsplit_triv \<Gamma>2 [typ1] \<Gamma>3 [typ2] \<Gamma>4
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, TSum ts T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', USum t v rs)
                           ; \<Xi>, \<xi> , (v # \<gamma>), L, K, C,  \<Gamma>3, \<tau> T\<turnstile> (\<sigma>', m) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Case x t m n) \<Down>! st"

| u_tt_sem_pres_case_nm : "\<lbrakk> ttsplit L K C \<Gamma> sps [] \<Gamma>1 [] \<Gamma>2
                           ; ttsplit_triv \<Gamma>2 [typ1] \<Gamma>3 [typ2] \<Gamma>4
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, TSum ts T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', USum t' v rs)
                           ; t' \<noteq> t
                           ; \<Xi>, \<xi> , (USum t' v rs # \<gamma>), L, K, C, \<Gamma>4, \<tau> T\<turnstile> (\<sigma>', n) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Case x t m n) \<Down>! st"

| u_tt_sem_pres_if      : "\<lbrakk> ttsplit L K C \<Gamma> sps [] \<Gamma>1 [] \<Gamma>2
                           ; ttsplit_triv \<Gamma>2 [] \<Gamma>3 [] \<Gamma>4
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, TPrim Bool T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', UPrim (LBool b))
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, (if b then \<Gamma>3 else \<Gamma>4), \<tau> T\<turnstile> (\<sigma>', if b then t else e) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, If x t e) \<Down>! st"

| u_tt_sem_pres_take    : "\<lbrakk> s = Boxed Writable l
                           ; ttsplit L K C \<Gamma> sps [] \<Gamma>1 [Some f_ty, Some (TRecord tak_fs s)] \<Gamma>2
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, TRecord ts s T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', UPtr p rp tyC)
                           ; \<sigma>' p = Some (URecord fs, tyC)
                           ; \<Xi>, \<xi> , (fst (fs ! f) # UPtr p rp tyC # \<gamma>), L, K, C, \<Gamma>2, \<tau> T\<turnstile> (\<sigma>', e) \<Down>! st
...
| u_tt_sem_pres_split   : "\<lbrakk> ttsplit L K C \<Gamma> sps [] \<Gamma>1 [Some t, Some u] \<Gamma>2
                           ; \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>1, TProduct t u T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', UProduct a b)
                           ; \<Xi>, \<xi> , (a # b # \<gamma>), L, K, C, \<Gamma>2, \<tau> T\<turnstile> (\<sigma>', e) \<Down>! st
                           \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Split x e) \<Down>! st"
...
shows   "\<lbrakk> \<xi>, \<gamma> \<turnstile>  (\<sigma>, e) \<Down>! (\<sigma>', v)
         ; \<Xi>, 0, [], {}, \<Gamma> T\<turnstile> e : \<tau>
         \<rbrakk> \<Longrightarrow> \<Xi>, \<xi> , \<gamma>, 0, [], {}, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, e) \<Down>! (\<sigma>', v)"
and     "\<lbrakk> \<xi>, \<gamma> \<turnstile>* (\<sigma>, es) \<Down>! (\<sigma>', vs)
         ; \<Xi>, 0, [], {}, (snd \<Gamma>) \<turnstile>* es : \<tau>s'
...
using assms proof (induct "(\<sigma>, e)"        "(\<sigma>', v )"
  ...
  case u_sem_let show ?case using u_sem_let.prems u_sem_let.hyps(1, 3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon> = Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_let.hyps(1)]
...
next
  ...
  case u_sem_letbang show ?case using u_sem_letbang.prems u_sem_letbang.hyps(1, 3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon> = Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_letbang.hyps(1)]
     , erule ttyping_eq_typing[THEN iffD2, OF exI])
    apply clarsimp
...
next
  ...
  case u_sem_split show ?case using u_sem_split.prems u_sem_split.hyps(1, 3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon>=Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_split.hyps(1)]
     , erule ttyping_eq_typing[THEN iffD2, OF exI])
    apply clarsimp
...
next
  ...
  case u_sem_if show ?case using u_sem_if.prems u_sem_if.hyps(1, 3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon>=Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_if.hyps(1)]
     , erule ttyping_eq_typing[THEN iffD2, OF exI])
...
next
  ...
  case u_sem_case_m show ?case using u_sem_case_m.prems u_sem_case_m.hyps(1, 3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon>=Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_case_m.hyps(1)]
     , erule ttyping_eq_typing[THEN iffD2, OF exI])
    apply (clarsimp simp: ttsplit_triv_def)
...
next
  ...
  proof (cases rule: ttyping.cases)
    ...
    moreover have "0, [], {} \<turnstile> snd \<Gamma> \<leadsto> snd t\<Gamma>1 | snd t\<Gamma>2"
    ...
    ultimately show "\<Xi>, \<xi>, \<gamma>, 0, [], {}, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Case x t m n) \<Down>! (\<sigma>', v)"
      using u_sem_case_nm.hyps
    proof (intro u_tt_sem_pres_case_nm)
      show "\<Xi>, \<xi>, \<gamma>, 0, [], {}, t\<Gamma>1, TSum ts T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>'', USum t' v' rs)"
        using u_sem_case_nm.hyps(2) matches1 u_sem_case_nm.prems ttyping_case by simp
    next
      have "\<Xi>, 0, [], {}, snd t\<Gamma>1 \<turnstile> x : TSum ts"
      ...
      then obtain r1' w1'
        ...
        using preservation_mono u_sem_case_nm.prems(2,4) matches1 u_sem_case_nm.hyps(1)       
        by blast

      show "\<Xi>, \<xi>, USum t' v' rs # \<gamma>, 0, [], {}, t\<Gamma>4, \<tau> T\<turnstile> (\<sigma>'', n) \<Down>! (\<sigma>', v)"
...
next
  case (u_sem_take \<xi> \<gamma> \<sigma> x \<sigma>'' p r' tyC fs f e)
  ...
  proof (cases rule: ttyping.cases)
    ...
    obtain \<Gamma>1 \<Gamma>2
      where "0, [], {} \<turnstile> snd \<Gamma> \<leadsto> \<Gamma>1 | \<Gamma>2"
    ...
    have "\<Xi>, 0, [], {}, \<Gamma>1 \<turnstile> x : TRecord ts s"
      using snd_t\<Gamma>3_is ttyping_imp_typing ttyping_take by fastforce
    then obtain r1' w1'
      where uptr_p_under_\<sigma>'': "\<Xi>, \<sigma>'' \<turnstile> UPtr p r' tyC :u TRecord ts s \<langle>r1', w1'\<rangle>"
        and r1'_sub: "r1' \<subseteq> r1"
        and frame1: "frame \<sigma> w1 \<sigma>'' w1'"
      using preservation_mono
    ...
    have "\<Xi>, \<xi>, fst (fs ! f) # UPtr p r' tyC # \<gamma>, 0, [], {}, t\<Gamma>4, \<tau> T\<turnstile> (\<sigma>'', e) \<Down>! (\<sigma>', v)"
    proof (cases taken)
      case Taken
      ...
      proof (intro u_sem_take.hyps(5))
        show "\<Xi>, \<sigma>'' \<turnstile> fst (fs ! f) # UPtr p r' tyC # \<gamma> matches snd t\<Gamma>4 \<langle>rf \<union> (r1a \<union> r2), wf \<union> (insert p w1a \<union> w2)\<rangle>"
        ...
        proof (simp only: snd_t\<Gamma>4_is append_Cons append.left_neutral, intro matches_ptrs_some[OF _ matches_ptrs_some])
          ...
          moreover have "r' = RRecord (map (type_repr \<circ> fst \<circ> snd) (ts[f := (n, t, taken)]))"
              ...
              by (metis (full_types))
          ultimately show "\<Xi>, \<sigma>'' \<turnstile> UPtr p r' tyC :u TRecord (ts[f := (n, t, taken)]) s \<langle>r1a, insert p w1a\<rangle>"
            using uptr_p_elim_lemmas ut_fs_taken_f r1'_is w1''_is
            apply simp
            by(intro u_t_p_rec_w'; simp add:ttyping_take map_nth_same)
        qed fast+
      qed simp+
    next
      case Present
      ...
      show ?thesis
        using u_sem_take.prems ttyping_take
      proof (intro u_sem_take.hyps(5))
        show "\<Xi>, \<sigma>'' \<turnstile> fst (fs ! f) # UPtr p r' tyC # \<gamma> matches snd t\<Gamma>4 \<langle>rf \<union> ((rf \<union> r1a) \<union> r2), {} \<union> (insert p w1a \<union> w2)\<rangle>"
        ...
        proof (simp only: snd_t\<Gamma>4_is append_Cons append.left_neutral, intro matches_ptrs_some[OF _ matches_ptrs_some])
          ...
          thus "\<Xi>, \<sigma>'' \<turnstile> UPtr p r' tyC :u TRecord (ts[f := (n, t, taken)]) s \<langle>rf \<union> r1a, insert p w1a\<rangle>"
            using uptr_p_elim_lemmas wf_empty r1'_is uptr_p_under_\<sigma>'' w1''_is Present by auto
        qed fast+
      qed simp+
    qed
    moreover have "\<Xi>, \<xi>, \<gamma>, 0, [], {}, t\<Gamma>3, TRecord ts s T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>'', UPtr p r' tyC)"
      using u_sem_take.hyps(2) u_sem_take.prems ttyping_take matches1 snd_t\<Gamma>3_is by auto
    ultimately show "\<Xi>, \<xi>, \<gamma>, 0, [], {}, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, Take x f e) \<Down>! (\<sigma>', v)"
...
next
  ...
  show ?case using u_sem_take_ub.prems u_sem_take_ub.hyps(1,3)
    ...
    apply (frule(2) preservation(1)[where \<epsilon>=Nil and \<tau>s=Nil, simplified, OF subst_wellformed_nothing, OF _ _ _ u_sem_take_ub.hyps(1)]
        , erule ttyping_eq_typing[THEN iffD2, OF exI])
    apply clarsimp
...
lemma u_tt_sem_pres_imp_u_sem:
  "\<Xi>, \<xi>, \<gamma>, 0, [], {}, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, x) \<Down>! (\<sigma>', uv)
...
lemma u_tt_sem_pres_type_wellformed:
  "\<lbrakk> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', x) \<rbrakk>
    \<Longrightarrow> \<forall>t. Some t \<in> set (snd \<Gamma>) \<longrightarrow> L, K, C \<turnstile> t wellformed"
...
lemma u_tt_sem_pres_type_wellformed2:
  "\<lbrakk> \<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', x) \<rbrakk>
    \<Longrightarrow> L, K, C \<turnstile>  \<tau> wellformed"
  by (induct rule: u_tt_sem_pres.induct,
    auto dest!: typing_to_wellformed)

lemma u_tt_sem_pres_preservation:
  "\<Xi>, \<xi>, \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> st \<Down>! st' \<Longrightarrow> L = 0 \<Longrightarrow> K = [] \<Longrightarrow> C = {} \<Longrightarrow>
  ...
  apply clarsimp
  apply (frule(4) preservation(1)[where \<epsilon>=Nil and \<tau>s=Nil and K=Nil, simplified, OF subst_wellformed_nothing])
  apply auto
  done

lemma u_tt_sem_pres_length:
  "\<Xi>, \<xi>, \<gamma>, 0, [], {}, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', x)
...
lemma let_elaborate_u_tt_sem_pres:
  assumes
    "\<Xi>, \<xi> , \<gamma>, L, K, C, \<Gamma>, \<tau> T\<turnstile> (\<sigma>, a) \<Down>! (\<sigma>', x)"
    "L = 0"
    "K = []"
    "C = {}"
    "proc_ctx_wellformed \<Xi>"
    "\<xi> matches-u \<Xi>"
  shows "\<Xi>, \<xi> , \<gamma>, L, K, C, (TyTrSplit (map (map_option (\<lambda>_. TSK_L)) (snd \<Gamma>)) [] (fst \<Gamma>)
    [Some \<tau>] TyTrLeaf, snd \<Gamma>), \<tau> T\<turnstile> (\<sigma>, Let a (Var 0)) \<Down>! (\<sigma>', x)"
  using assms
proof (intro u_tt_sem_pres_let[OF ttsplitI])
  show "ttsplit_inner L K C (map (map_option (\<lambda>_. TSK_L)) (snd \<Gamma>)) (snd \<Gamma>) (snd \<Gamma>) (replicate (length (snd \<Gamma>)) None)"
...
next
  show "\<Xi>, \<xi>, x # \<gamma>, L, K, C, (TyTrLeaf, Some \<tau> # replicate (length (snd \<Gamma>)) None), \<tau> T\<turnstile> (\<sigma>', Var 0) \<Down>! (\<sigma>', x)"
    using assms
    apply (auto
