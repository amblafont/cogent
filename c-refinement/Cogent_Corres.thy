...
lemma corres_take_boxed':
  ...
  assumes x_boxed:
    "\<And> fs ty r w.
    \<lbrakk>(\<sigma>,s)\<in>srel; \<sigma> p = Some (URecord fs, ty);
...
lemma corres_take_boxed:
  ...
  assumes shareable_or_taken: "S \<in> k \<or> taken = Taken"
  assumes x_boxed:
  "\<And> fs ty r w. \<lbrakk>(\<sigma>, s)\<in> srel; \<sigma> p = Some (URecord fs, ty); \<Xi>, \<sigma> \<turnstile> UPtr p repr :u TRecord typ sgl \<langle>r, w\<rangle>\<rbrakk> \<Longrightarrow>
...
lemma corres_let_put_boxed:
  ...
  "\<And>fs ty r w r' w'.
    ...
    ; \<Xi>, \<sigma>(p := Some (URecord (fs [f := (\<gamma>!e,  snd (fs ! f))]), ty)) \<turnstile> UPtr p repr :u TRecord typ sgl \<langle>r', w'\<rangle>
...
proof (clarsimp simp: corres_def in_monad snd_bind snd_modify snd_state_assert, auto)
  ...
  then obtain r1' w1p'
    where preserve_mono_on_put_lemmas:
      "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile> UPtr p repr :u TRecord (typ[f := (fst (typ ! f), fst (snd (typ ! f)), Present)]) sgl \<langle>r1', w1p'\<rangle>"
      "r1' \<subseteq> r1"
      "frame \<sigma> w1 (\<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty))) w1p'"
  ...
  obtain w1' ptrl
    where rec_elim1_lemmas:
        "w1p' = insert p w1'"
        "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile>* (fs[f := (\<gamma> ! e, snd (fs ! f))]) :ur typ[f := (fst (typ ! f), fst (snd (typ ! f)), Present)] \<langle>r1', w1'\<rangle>"
  ...
  obtain r1'' w1''
    where taken_field1_lemmas:
      "r1'' \<subseteq> r1'"
      "w1'' \<subseteq> w1'"
      "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile>* fs[f := (\<gamma> ! e, snd (fs ! f))] :ur typ \<langle>r1'', w1''\<rangle>"
  ...
  then have "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile>
  ...
  then have x_boxed_lemmas:
    "is_valid s x'"
    "(\<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)), h s) \<in> srel"
    using x_boxed assms' uval_typing1_elim_lemmas \<gamma>_x rec_elim1_lemmas use\<Gamma>3'_lemmas
    by metis+

  have upd_matches2:
    "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile> \<gamma> ! x # \<gamma> matches Some (TRecord (typ[f := (fst (typ ! f), fst (snd (typ ! f)), Present)]) sgl) # \<Gamma>2 \<langle>r1' \<union> r2, (insert p w1') \<union> w2\<rangle>"
  proof (intro matches_ptrs_some)
    show "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile> \<gamma> ! x :u TRecord (typ[f := (fst (typ ! f), fst (snd (typ ! f)), Present)]) sgl \<langle>r1', insert p w1'\<rangle>"
      using preserve_mono_on_put_lemmas rec_elim1_lemmas sigil_wr \<gamma>_x
      by argo
  next
    show "\<Xi>, \<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)) \<turnstile> \<gamma> matches \<Gamma>2 \<langle>r2, w2\<rangle>"
  ...
  have smaller_corres:
    "\<not> snd (y' (h s))"
    "\<And>r' s'. (r', s') \<in> fst (y' (h s)) \<Longrightarrow> \<exists>\<sigma>' r. \<xi>, \<gamma> ! x # \<gamma> \<turnstile> (\<sigma>(p \<mapsto> (URecord (fs[f := (\<gamma> ! e, snd (fs ! f))]), ty)), y) \<Down>! (\<sigma>', r) \<and> (\<sigma>', s') \<in> srel \<and> val_rel r r'"
...
lemma u_sem_put':
  "\<xi>', \<gamma>' \<turnstile> (\<sigma>', x) \<Down>! (\<sigma>'', r) \<Longrightarrow>
   \<sigma>'' p = Some (URecord fs, ty) \<Longrightarrow>
   r = UPtr p repr \<Longrightarrow>
   \<xi>', \<gamma>' \<turnstile> (\<sigma>'', e) \<Down>! (\<sigma>''', e') \<Longrightarrow>
   \<xi>', \<gamma>' \<turnstile> (\<sigma>', Put x f e) \<Down>! (\<sigma>'''(p \<mapsto> (URecord (fs[f := (e', snd (fs ! f))]), ty)), r)"
...
lemma corres_put_boxed:
  ...
  "\<And>fs ty r w r' w'.
    \<lbrakk>(\<sigma>,s)\<in> srel; \<sigma> p = Some (URecord fs, ty);
    \<Xi>, \<sigma> \<turnstile> UPtr p repr :u TRecord typ sgl \<langle>r, w\<rangle>;
    \<Xi>, \<sigma>(p := Some (URecord (fs [f := (\<gamma>!e, snd (fs ! f))]), ty)) \<turnstile> UPtr p repr :u TRecord typ sgl \<langle>r', w'\<rangle>\<rbrakk> \<Longrightarrow>
   is_valid s x' \<and> val_rel (UPtr p repr) x' \<and> (\<sigma>(p := Some (URecord (fs [f := (\<gamma>!e, snd (fs ! f))]), ty)), h s) \<in> srel"
  shows "corres srel
           (Put (Var x) f (Var e))
  ...
  apply (frule(1) x_boxed)
   ...
   apply force
  apply clarsimp
  apply (rule_tac x="\<sigma>(p := Some (URecord (fs [f := (\<gamma>!e, snd (fs ! f))]), tya))" in exI)
...
lemma corres_member_boxed:
  assumes
    ...
    "\<And>fs ty r w. \<lbrakk>(\<sigma>, s)\<in> srel; \<sigma> (ptr_val x') = Some (URecord fs, ty);
